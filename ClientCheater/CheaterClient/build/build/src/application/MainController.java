...
import application.cheater.BoggleSolver;
import application.cheater.Coordinate;
import application.cheater.SolutionCheat;
import application.request.CMDRequestEnum;
import application.request.ClientRequest;
import application.request.TraitementRequetes;
import application.respons.TraitementReponse;
import application.utilities.MyTimer;
import application.utilities.Score;
...
public class MainController implements Observer {
	...
	// ClientRandomFunctions
	public void randomChatBehavior() {
		int randomNum = ThreadLocalRandom.current().nextInt(1, 5);
		switch (randomNum) {
		case 1:
			chatMessage.setText("C'est moi le meilleur");
			chatButton.fire();
			break;
		case 2:
			chatMessage.setText("je vais surement gagner");
			chatButton.fire();
			break;
		case 3:
			chatMessage.setText("Vous avez aucune chance");
			break;
		case 4:
			chatMessage.setText("Vous etes des nuls");
			break;
		default:
			break;
		}
	...
	public void tricher(ActionEvent e) {
		SolutionCheat sol = solver.getSolution();
		StringBuilder trajectoire = new StringBuilder();
		if(sol.getTrajectoire() != null && sol.getTrajectoire().size()>0)
		for (Coordinate cord : sol.getTrajectoire()) {
			trajectoire.append(get(cord.getX(), cord.getY()).getId());
			get(cord.getX(), cord.getY()).fire();
		}
		tricheText.setText(sol.getMot());
